...
<head>
    ...
    <title>GENESIS | Y10 MASTER (B72 - FOUNDER RESTORED)</title>
    <script src="https://na01.safelinks.protection.outlook.com/?url=https%3A%2F%2Fcdn.jsdelivr.net%2Fnpm%2Fexifreader%404.12.0%2Fdist%2Fexif-reader.min.js&data=05%7C02%7C%7C415bc793afc24f5b5f6c08decb015507%7C84df9e7fe9f640afb435aaaaaaaaaaaa%7C1%7C0%7C639171403726704478%7CUnknown%7CTWFpbGZsb3d8eyJFbXB0eU1hcGkiOnRydWUsIlYiOiIwLjAuMDAwMCIsIlAiOiJXaW4zMiIsIkFOIjoiTWFpbCIsIldUIjoyfQ%3D%3D%7C0%7C%7C%7C&sdata=VrN8xVN3gzO1pzpPAsU1teMe%2B%2Fjf%2BIDUflaiYTfWLyc%3D&reserved=0"></script>
    <style>
...
<body style="background:#121212; min-height:100vh; color:white; font-family:sans-serif; width:100%; margin:0; display:flex; flex-direction:column;">

<div id="genesis-root" style="display: none;">
    <div style="padding:10px 20px; background:#1a1a1a; border-bottom:1px solid #333; display:flex; justify-content:space-between; align-items:center;">
        ...
        <div style="display:flex; align-items:center; gap:20px; background:#252525; padding:5px 15px; border-radius:20px; border:1px solid #444;">
            ...
            <div class="upload-container">
                <button type="button" id="unified-upload-btn" onclick="triggerWebUpload()" style="background: #00d4ff; color: black; border: none; padding: 5px 15px; border-radius: 20px; font-weight: bold; cursor: pointer;">Choose File</button>
        ...
        <div style="display:flex; gap:10px;">
            ...
            <a href="https://na01.safelinks.protection.outlook.com/?url=https%3A%2F%2Fwww.youtube.com%2F%40OspreonLLC&data=05%7C02%7C%7C415bc793afc24f5b5f6c08decb015507%7C84df9e7fe9f640afb435aaaaaaaaaaaa%7C1%7C0%7C639171403726733288%7CUnknown%7CTWFpbGZsb3d8eyJFbXB0eU1hcGkiOnRydWUsIlYiOiIwLjAuMDAwMCIsIlAiOiJXaW4zMiIsIkFOIjoiTWFpbCIsIldUIjoyfQ%3D%3D%7C0%7C%7C%7C&sdata=gZUGZC7%2FvkG8cWgwM6fplPuZq5ZD%2FqRRd3XyjeoDDk4%3D&reserved=0" target="_blank" class="social-btn yt-btn">YouTube</a>
            <a href="https://na01.safelinks.protection.outlook.com/?url=https%3A%2F%2Fwww.facebook.com%2F&data=05%7C02%7C%7C415bc793afc24f5b5f6c08decb015507%7C84df9e7fe9f640afb435aaaaaaaaaaaa%7C1%7C0%7C639171403726756033%7CUnknown%7CTWFpbGZsb3d8eyJFbXB0eU1hcGkiOnRydWUsIlYiOiIwLjAuMDAwMCIsIlAiOiJXaW4zMiIsIkFOIjoiTWFpbCIsIldUIjoyfQ%3D%3D%7C0%7C%7C%7C&sdata=KZvUaXGJKtIwoSWZfEeTAXUXhGfj25GjcWNySnYuWTc%3D&reserved=0" target="_blank" class="social-btn fb-btn">Facebook</a>
        </div>
    </div>
    ...
    <div id="viewport" style="width:100%; overflow:hidden; background:#000; position:relative; min-height:550px; border-bottom:2px solid #333; display:flex; justify-content:center; align-items:flex-start;">
        ...
        <div id="tool-dock" style="position:absolute; background:rgba(26,26,26,0.98); border:2px solid #00d4ff; border-radius:10px; padding:10px; flex-direction:column; gap:8px; z-index:5000; pointer-events:none; box-shadow: 0 10px 30px rgba(0,0,0,0.8); width:110px; display:none;">
                ...
                <div id="content-viewport" style="flex-grow: 1; position: relative;">
                    <div id="video-content" style="width:100%; height:100%; display:flex; align-items:center; justify-content:center;">
                        <video id="ad-video" width="100%" height="100%" autoplay loop muted playsinline style="object-fit:cover;">
                            <source src="https://na01.safelinks.protection.outlook.com/?url=https%3A%2F%2Fwww.pexels.com%2Fdownload%2Fvideo%2F7581208%2F&data=05%7C02%7C%7C415bc793afc24f5b5f6c08decb015507%7C84df9e7fe9f640afb435aaaaaaaaaaaa%7C1%7C0%7C639171403726777126%7CUnknown%7CTWFpbGZsb3d8eyJFbXB0eU1hcGkiOnRydWUsIlYiOiIwLjAuMDAwMCIsIlAiOiJXaW4zMiIsIkFOIjoiTWFpbCIsIldUIjoyfQ%3D%3D%7C0%7C%7C%7C&sdata=EeUEVnPsRKjkhGJOabMg0oLL%2Bu6mJKnK%2FQT0mukrEzQ%3D&reserved=0" type="video/mp4">
                        </video>
                    </div>
...
<script>
    // Security Verification on web load
    window.onload = function() {
        let entered_pin = prompt("ENTER SECURITY VERIFICATION PIN:");
        let authorized_pins = ["12345", "67890", "1234", "70426"];

        if (authorized_pins.includes(entered_pin)) {
            alert("Verification Successful. Preparing canvas...");
            document.getElementById('genesis-root').style.display = 'block';
            if (entered_pin === "70426") {
                alert("TRAINEE PATHWAY UNLOCKED: Pre-designated training library access only.");
            }
            // Trigger load/initialization
            initB72();
        } else {
            alert("Invalid Pin Entered. Access Restricted.");
            document.body.innerHTML = "<h1 style='color:red; text-align:center; margin-top:50px;'>ACCESS RESTRICTED</h1>";
        }
    };
    ...
    function handleNadirUpload(event) {
        ...
        reader.onload = function(e) {
            ...
            // Update photo id / signatures dynamically
            let NadirB64 = e.target.result;
            img.src = NadirB64;
            document.getElementById('photo-id-box').value = "SIG-" + event.target.files[0].size + "-NAD";
            interrogatePhoto(event.target.files[0], event.target.files[0].name);
        };
    ...
    window.ruthSuggest = function(message, type = 'audit') {
        ...
        if (overlay && content) {
            ...
            if (targetVoice) voice.voice = targetVoice;
    ...
    function initB72() {
        ...
        ctx.fillText("Upload or Choose an image to begin", 130, 200);
    }
    ...
    function executeAttentionFlash() {
        ...
        if (container) {
            container.classList.add('pitch-alert');
            setTimeout(() => {
                container.classList.remove('pitch-alert');
            }, 8000);
        }
    }
    ...
    function triggerScaleFlash() {
        ...
        if (!scaleFlashTriggered) {
            executeAttentionFlash();
            scaleFlashTriggered = true;
        }
    }

    window.accumulateJ18 = function() {
        const ev = parseFloat(document.getElementById('val-eave').innerText) || 0;
        const rk = parseFloat(document.getElementById('val-rake').innerText) || 0;
        const rd = parseFloat(document.getElementById('val-ridge').innerText) || 0;
        const hp = parseFloat(document.getElementById('val-hip').innerText) || 0;
        const vy = parseFloat(document.getElementById('val-valley').innerText) || 0;
        const sw = parseFloat(document.getElementById('val-sw').innerText) || 0;
        const ew = parseFloat(document.getElementById('val-ew').innerText) || 0;
        const totalSqFt = parseFloat(document.getElementById('total-sqft').innerText) || 0;
        const metalWidthIn = parseFloat(document.getElementById('metal-width-box').value) || 0;
    ...
    window.print2DBlueprint = function() {
        ...
        savedFacets.forEach((f) => {
            pCtx.fillStyle = "rgba(40, 44, 52, 0.25)";
            pCtx.fill();
    ...
    async function interrogatePhoto(fileObject, fileName) {
        try {
            const buffer = await fileObject.arrayBuffer();
            const tags = await ExifReader.load(buffer, { expanded: true });
           
            const decoder = new TextDecoder('utf-8');
            const rawText = (await fileObject.text()).substring(0, 150000).toUpperCase();
            ...
            // Populate gallery feeds
            gallery[fileName] = URL.createObjectURL(fileObject);
